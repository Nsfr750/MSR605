...
from PyQt6.QtWidgets import (
    ...
    QLabel, QLineEdit, QPushButton, QTextEdit, QComboBox, QCheckBox, QGroupBox,
    QMessageBox, QFileDialog, QDialog, QDialogButtonBox, QStatusBar, QSplitter,
    QTableWidget, QTableWidgetItem, QHeaderView, QToolBar, QToolButton, QMenu,
    QSystemTrayIcon, QStyle, QMenuBar, QSizePolicy, QSpacerItem, QFrame,
    QRadioButton, QButtonGroup, QScrollArea, QInputDialog, QPlainTextEdit, 
    QTextBrowser
)
...
class GUI(QMainWindow):
    ...
    def save_settings(self):
        ...
        if not hasattr(self, 'language_manager') or not hasattr(self.language_manager, 'settings'):
            print("  Error: language_manager or language_manager.settings not available")
            return
            
        settings = self.language_manager.settings
        print(f"  Settings file: {settings.fileName()}")
        
        # Save application settings
        settings.setValue("auto_save", self.__auto_save_database)
        settings.setValue("allow_duplicates", self.__enable_duplicates)
        settings.setValue("coercivity", self.__coercivity)
        
        print(f"  Saved settings - auto_save: {self.__auto_save_database}, "
        ...
        if hasattr(self, 'saveState'):
            ...
            print(f"  Saved window state: {state}")
    ...
    def load_settings(self):
        ...
        # Load application settings
        self.__auto_save_database = settings.value("auto_save", False, type=bool)
        self.__enable_duplicates = settings.value("allow_duplicates", False, type=bool)
        self.__coercivity = settings.value("coercivity", "hi", type=str)
        
        print(f"  Loaded settings - auto_save: {self.__auto_save_database}, "
              f"allow_duplicates: {self.__enable_duplicates}, "
              f"coercivity: {self.__coercivity}")
        
        # Load window geometry
        if hasattr(self, 'restoreGeometry') and settings.contains("geometry"):
            geometry = settings.value("geometry")
            self.restoreGeometry(geometry)
            print(f"  Restored window geometry")
        if hasattr(self, 'restoreState') and settings.contains("windowState"):
            state = settings.value("windowState")
            self.restoreState(state)
            print(f"  Restored window state")
    ...
    def reset(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"Failed to reset MSR605: {str(e)}"
            )
    ...
    def show_sponsor(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"Failed to show sponsor dialog: {str(e)}"
            )
    ...
    def view_database(self):
        ...
        self.db_table.setHorizontalHeaderLabels(["ID", "Date", "Track 1", "Track 2", "Track 3", "Notes"])
    ...
    def setup_database_tab(self):
        ...
        self.db_table.setHorizontalHeaderLabels(["Date", "Track 1", "Track 2", "Track 3", "Notes"])
    ...
    def setup_settings_tab(self):
        ...
        self.hi_coercivity.toggled.connect(
            lambda: self.set_coercivity('hi' if self.hi_coercivity.isChecked() else 'lo')
        )
    ...
    def connect_to_msr605(self):
        ...
        if not port or port == "No ports found":
            QMessageBox.critical(self, "Error", "No port selected or no ports available")
            return
    ...
    def set_coercivity(self, coercivity):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Error",
                f"Failed to set coercivity: {str(e)}"
            )
    
    def read_card(self):
        """Read data from a magnetic stripe card."""
        if not self.__connected or self.__msr is None:
            QMessageBox.warning(self, "Not Connected", "Please connect to the MSR605 first")
            return
    ...
    def write_card(self):
        ...
        if not any(tracks):
            QMessageBox.warning(self, "No Data", "Please enter data for at least one track")
            return
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Write Error",
                f"Failed to write to card: {str(e)}"
            )
    ...
    def save_to_database(self):
        ...
        try:
            ...
            if not self.__enable_duplicates:
                try:
                    self.__db_cursor.execute(
                        ...
                        (self.__tracks[0] or None, self.__tracks[1] or None, self.__tracks[2] or None)
                    )
            ...
            try:
                ...
                if hasattr(self, 'db_table') and hasattr(self.db_table, 'isVisible') and self.db_table.isVisible():
                    self.refresh_database()
    ...
    def refresh_database(self):
        ...
        try:
            # Get the search text
            search_text = self.db_search.text().lower() if hasattr(self, 'db_search') else ""
            ...
            try:
                ...
                QMessageBox.critical(
                    self,
                    "UI Error",
                    f"Failed to update database view: {str(e)}"
                )
    ...
    def init_database(self):
        ...
        try:
            ...
            db_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'card_database.db'))
