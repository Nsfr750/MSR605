...
def parse_all_tracks(tracks):
    ...
    result['summary'] = {}
    if t1:
        result['summary']['track1'] = f"Card: {t1.get('card_number','')} | Name: {t1.get('name','')} | Exp: {t1.get('expiration','')}"
    if t2:
        result['summary']['track2'] = f"Card: {t2.get('card_number','')} | Exp: {t2.get('expiration','')} | Code: {t2.get('service_code','')}"
    if t3:
        result['summary']['track3'] = f"Len: {t3.get('length','')} | Raw: {t3.get('raw','')}"
    return result
