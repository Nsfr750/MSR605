...
class VoiceControl(QObject):
    ...
    def start_listening(self) -> None:
        ...
        if not self.recognizer or not self.microphone:
            logger.error("Speech recognition not available. Install SpeechRecognition package.")
            return
...
def create_default_voice_controls(gui) -> 'VoiceControl':
    ...
    if voice_control.recognizer is not None:
        ...
        commands = {
            ...
            # Navigation commands
            "show read tab": lambda: getattr(gui, 'tabs', None) and gui.tabs.setCurrentIndex(0),
            "show write tab": lambda: getattr(gui, 'tabs', None) and gui.tabs.setCurrentIndex(1),
            "show database": lambda: getattr(gui, 'tabs', None) and gui.tabs.setCurrentIndex(2),
            "show settings": lambda: getattr(gui, 'tabs', None) and gui.tabs.setCurrentIndex(3),
